...
ggplot(rpd, aes(year, n, color = drug)) + 
  ...
  annotate("text", label = "End of\n'War on Drugs'", x = 2009.25, y = 525, color = "gray30", size = 3) +
  annotate("text", label = "Marijuana 1st\nLeagalized", x = 2012.8, y = 525, color = "gray30", size = 3) +
...
ggplot(pud1, aes(use_yr, year, fill = drug)) +
...
#barely any ethnicity differences
pd_sum_eth <- pd1_rev %>% 
  filter(year == 2015) %>% 
...
ggplot(pd1_rev_sum, aes(avg_age, education)) + 
  ...
  labs(title = "Does age of first substance use differ based on education status?",
       x = "Average age of first use",
       y = "") +
