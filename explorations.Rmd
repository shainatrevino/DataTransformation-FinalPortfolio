...
* try facet by sex or gender or income (2levels)
...
ggplot(pud, aes(drug, days)) +
  geom_jitter(color = factor(year))
```

```{r}
pud11 <- pu_01 %>% 
  janitor::clean_names() %>% 
  filter(rxd240b == "LORAZEPAM" |
         rxd240b == "ALPRAZOLAM" |
         rxd240b == "TRIAZOLAM" | #ONLY 1 CASE IN 15
         rxd240b == "CHLORDIAZEPOXIDE" | # DONT INCLUDE
         rxd240b == "DIAZEPAM" |
         rxd240b == "CLONAZEPAM") %>% 
  select(seqn, rxd240b, rxd260, rxd295) %>% 
  rename(drug = rxd240b,
         days = rxd260, 
         count = rxd295)
...
pud111 <- left_join(pud11, to_join, by = "seqn") %>% 
  naniar::replace_with_na(replace = list(days = 99999))

ggplot(pud111, aes(drug, days)) +
  geom_jitter()
```
