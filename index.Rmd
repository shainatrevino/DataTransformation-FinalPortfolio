...
The data presented in this portfolio came from three questionnaires: 
...
Column {data-width=550 data-commentary-height=300}
-----------------------------------------------------------------------
...
* Change color of error bars and point

* Back to default colors for point estimate
...
I know it is double encoding, but I also think the visual is really appealing and sort of draws you in with the color coding by education level on a dark background (just something to consider). 
...
> In the first revision of this plot, I substituted education groups as a proxy for education levels, since there are less inherent categories. Also, I summarized and plotted a mean age of first use by substance type, income, and sex instead of the previous count. As you can see, there are not many observable differences related to sex. So, to reduce grouping variables and make the plot more succinct, I will take out sex from these data in the next rendition. Although this plot does show differences in average age of use, it doesn't show any uncertainty around the estimate or the actual distribution of the data that would be interpretable and helpful for an academic audience. 
...
> releveled education categories
jitter with point estimate using different datasets
#can't really see differences in previous plot - make jitter with point estimate and se bar
# add jitter from other dataset (before point)
#coord flip to see labels better and differences in outcome
...
* have  education level on x axis and facet by drug to more clearly see differences in education level for each drug.
* coord flip?
* fct_reorder levels of education manually according to progression
* filter out NAs
* plot jittered raw distribution with point and uncertaintly over it
* decresed alpha a lot because of overplotting
...
> # added ncol = 1 to see more of variability 
# geared more towards general audience, but since this is for academic can use distribution which I like better. 
#limit x axis
...
* add label for average age and nudge so you can see them
* could try stat density
* tried just point and uncertainty, but had to skew axis too much, liked having distribution of points showing extreme values and differences in number of people reporting (e.g., data points on jittered distribution)
* force one column to easier to compare accross drugs and expand x axis that is years
* added title and caption, labels since skewed axis
PR - #I really like your use of the dark background and the blue band to display the error bars. I think choosing to turn your error bars into a band as opposed to the lines of error bars over jittered data points displays the patterns of your data clearer, so really good decision! 
...
* back to original scale and distribution of points (see above comment)
* change colors and theme layout
  * darkened oke theme
* changed title
* added caption
* double encoded with color
